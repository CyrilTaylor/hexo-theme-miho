(function (w, d) {
	...
	var Blog = {
		...
		showToc: function (scrollTop) {
			...
			var offset = 0;
			var tocsHeight =
				tocsList[tocsList.length - 1].getBoundingClientRect().y -
			...
			offset =
				offset < tocsHeight - clientHeight * 0.236
					? offset
					: tocsHeight - clientHeight * 0.236;
		...
		reward: function () {
			...
			if (reward) {
				...
				rewardCheck.click(function () {
					$(this)
						.addClass("checked")
						.siblings(rewardCheck)
						.removeClass("checked");
					rewardCode.attr(
						"src",
						$(this).attr("data-id") === "wechat"
							? this.dataset.wechat
							: this.dataset.alipay
					);
				});
	...
	searchWrap.click(function (e) {
		if (
			e.target.className == "search-item-li" ||
			e.target.className == "search-item-li-title"
		) {
			return true;
		}
	...
	$("#sidebar-category").click(function (e) {
		if (
			sideMenuBoxIsOpen &&
			categories.attr("style") == "display: block;"
		) {
